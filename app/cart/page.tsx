...
export default function Cart() {
  ...
  */
  useEffect(() => {
    ...
    setCartItems(JSON.parse(localStorage.getItem("cart") || "[]") as CartItem[])
  }, []);
  ...
  useEffect(() => {
    ...
    getSession().then(session => {
      if (session && session.user?.name) {
        return getUser(session.user.name).then(fetchedUser => { setUser(fetchedUser) });
      }
    })
  ...
  */
  const onCheckout = async () => {
    // If there is no user logged in, return
    if (!user) {
      return;
    }

    // Get the cart items from local storage and parse the cart items as a CartItem array
    setCartItems(JSON.parse(localStorage.getItem("cart") || "[]") as CartItem[])

    // Set the cart checkout loading state to true
    setCartCheckoutLoading(true);
    
    // For each item in the cart, purchase the item using the purchaseItem function. Once all 
    ...
    Promise.all(cartItems.map((item) => purchaseItem(item.id, item.quantity, user.username)))
      .then((purchaseResults) => {
        if (purchaseResults.every((result) => result !== null)) {
