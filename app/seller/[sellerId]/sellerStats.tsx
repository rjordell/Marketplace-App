...
export default function SellerStats({ seller }: { seller: User }) {
  ...
  */
  useEffect(() => {
    // Get and update the seller's stats. 
    Promise.all([
      getUserNumberOfSales(seller.username),
      getUserNumberOfPurchases(seller.username),
      getStarsForTarget(seller.username),
      getNumberOfRatingsForTarget(seller.username),
    ]).then(([sales, purchases, averageStars, ratings]) => {
