...
export default function CreateListingModal() {
  ...
  */
  useEffect(() => {
    if (session && session.user?.name) {
      getUser(session.user.name).then((fetchedUser) => { setUser(fetchedUser) })
    }
  }, [session]);
  ...
  */
  const onCreateListing = async () => {
    ...
    // Parse the quantity and listing price as an integer and float, respectively. If either is invalid, set error to true and return.
    ...
    addNewItem({
      ...
      // Set loading to false and success to true.
      setLoading(false);
      setSuccess(true);
      ...
      setQuantity("1");
      setCategory(null);
      // Redirect the user to their page with the listings tab selected.
