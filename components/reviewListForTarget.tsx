...
export default function ReviewListForTarget({ target }: { target: string }) {
  ...
  */
  useEffect(() => {
    ...
    // Get the reviews for the target and update the reviews state.
    getReviewsForTarget(target).then((fetchedReviews) => { 
  ...
  return (
    <div className="flex flex-col gap-4 items-center py-4 h-full w-full">
      {
        // Map through each of the reviews and display a ReviewCard component for each one.
