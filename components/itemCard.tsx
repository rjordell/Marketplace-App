...
export default function ItemCard({ itemId }: { itemId: string }) {
  ...
  */
  useEffect(() => {
    // Get the current item and updates it
    getItemById(itemId).then((fetchedItem) => { setItem(fetchedItem) });
    
    // Get the user if the user is logged in, check if the item is on the user's watch list. Update the user, and isWatched states.
    if (session && session.user?.name) {
      getUser(session.user.name).then((fetchedUser) => {
        setUser(fetchedUser);
        isUserWatchingItem(fetchedUser.username, itemId).then((watching) => { setIsWatched(watching) });
      });
    }
  }, [itemId, session]);
