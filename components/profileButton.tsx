...
export default function ProfileButton() {
  ...
  */
  useEffect(() => {
    // could check if status === "authenticated" instead but this is a bit more concise
    if (session && session.user?.name) {
      getUser(session.user.name).then((fetchedUser) => {
        setUser(fetchedUser);
        getUserShopBalance(fetchedUser.username).then((balance) => { setShopBalance(balance?.toString() ?? "0") })
      });
    }
  }, [session]);
  ...
  */
  const onSignUp = async () => {
    createUser(usernameInput, passwordInput).then(() => { signIn("credentials", { username: usernameInput, password: passwordInput }) })
  };
