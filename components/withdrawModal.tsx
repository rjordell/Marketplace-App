...
export default function WithdrawModal() {
  ...
  */
  useEffect(() => {
    // Get the withdraw URL query parameter, and if it exists, set the showDialog state to true.
    const queryParams = new URLSearchParams(window.location.search);
    if (queryParams.get("withdraw")) {
      setShowDialog(true);
    }

    // If the session exists and the user is logged in, get the user from the database and use the user's balance to update the balance state.
  ...
  */
  const onWithdraw = async () => {
    ...
    // Return if the session or the session name does not exist.
    if (!session || !session.user?.name) {
      return;
    }

    // Get the user from the database, and if the user does not exist, set error to true and return.
    getUser(session.user.name).then((user) => {
      ...
      // Withdraw the balance from the shop and set success to true.
      const parsedAmount = parseInt(amount);
      if (!isNaN(parsedAmount)) {
        withdrawBalanceFromShop(user.username, parsedAmount).then(() => { 
          setSuccess(true);
          // Remove the withdraw URL query parameter from the URL and set showDialog to false.
          window.history.replaceState(null, "", window.location.pathname); // not sure if this does anything?
          setShowDialog(false);
          setLoading(false); 
        });
      }
